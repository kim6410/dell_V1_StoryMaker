...
5. 이번 작업과 무관한 기존 미추적 파일이 발견되면 임의 삭제하지 않습니다. 다만 이를 무시한 채 새 미추적 파일을 추가로 쌓아서는 안 되며, 새로 생성한 파일은 반드시 이번 작업 안에서 정리합니다.

6. 작업 완료 판정은 기능 검증뿐 아니라 **이번 작업으로 새 미추적 파일이 증가하지 않았음**이 확인되어야 합니다. 작업 시작 시 미추적 파일 목록을 기준값으로 기록하고, 종료 시 동일 명령으로 비교합니다.

7. 사용자가 명시적으로 미추적 파일 생성을 허용하더라도 이는 작업 중 임시 생성을 허용한다는 뜻일 뿐, 작업 종료 후 방치를 허용하는 것으로 해석하지 않습니다.

이 규칙을 지키지 않은 작업은 기능이 정상이어도 완료로 보고하지 않습니다.
