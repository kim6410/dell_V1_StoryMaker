...
## 최우선 강제 규칙: 미추적 파일 생성·방치 금지

이 저장소에서 작업하는 모든 사람과 AI는 작업 중 임시 파일, 패치 스크립트, 검사 결과, 빌드 산출물, 새 기능 파일을 생성할 수 있으나, **작업 종료 시 미추적 상태로 방치해서는 안 됩니다.**

다음 규칙은 예외 없이 적용합니다.

1. 작업 시작 전과 종료 직전에 반드시 아래 명령으로 미추적 파일을 확인합니다.
...
2. 새로 만든 파일은 작업 종료 전에 반드시 다음 중 하나로 분류하고 처리합니다.

- 실제 운영·소스에 필요한 파일: 정확한 경로를 지정해 Git에 추가하고 커밋합니다.
- 일회성 패치·검사·중간 산출물: 검증이 끝나면 즉시 삭제합니다.
- 보존이 필요한 참고자료·대용량 결과물: 저장소 밖의 승인된 `Backup` 또는 비공개 백업 경로로 이동합니다.
- 반복 생성되는 빌드·캐시 파일: 현재 서비스 참조 여부를 확인한 뒤 `.gitignore`에 반영하거나 빌드 정리 절차를 추가합니다.

3. 다음 행동은 금지합니다.

- 임시 파일을 저장소 루트나 소스 폴더에 만든 뒤 그대로 종료하는 행위
- `patch_*.py`, `*_excerpt.txt`, `served-*.js`, 검사 JSON, 임시 복사본을 정리하지 않는 행위
- 빌드 전 기존 산출물을 정리하지 않아 해시형 파일을 계속 누적하는 행위
- 실제 운영에 필요한 새 파일을 Git에 추가하지 않은 채 서비스에서만 참조하는 행위
- 미추적 파일의 용도와 처리 상태를 확인하지 않고 `git clean` 또는 일괄 삭제하는 행위

4. 작업 종료 보고에는 반드시 다음 내용을 포함합니다.

- 이번 작업에서 새로 생성한 파일 목록
- Git에 추가한 파일 목록
- 삭제하거나 백업 폴더로 이동한 임시 파일 목록
- 작업 종료 시 남아 있는 미추적 파일 수
- 남은 미추적 파일이 있다면 각 파일의 용도와 남겨둔 이유

5. 작업 시작 시 기존 미추적·미커밋 파일이 발견되면 이를 단순 보존 상태로 남겨두지 않습니다. 먼저 파일별 용도와 소유 작업을 확인하고, 실제 소스·문서·설정이면 정확한 경로만 지정해 커밋하고, 런타임 산출물·캐시·모델·DB·비밀정보처럼 Git 제외 대상이면 `.gitignore` 규칙을 보완해 **`git status --short`에 더 이상 나타나지 않도록 정상화**합니다. 다른 작업자가 현재 수정 중인 파일이라 임의 처리가 불가능한 경우에는 새 작업을 완료 처리하지 않고 해당 파일의 소유 작업이 끝날 때까지 보류합니다.

6. 작업 완료 판정은 기능 검증만으로 끝나지 않습니다. **종료 시 `git status --short` 출력이 0줄이어야 하며, 미추적 파일 0개·미커밋 변경 0개가 강제 조건입니다.** 단순히 "이번 작업에서 늘어나지 않았다"는 이유로 기존 dirty 상태를 허용하지 않습니다.

7. Push가 포함되는 모든 작업은 반드시 `커밋 → Push → 원격 검증 → 최종 clean 확인` 순서로 끝냅니다. Push 후 로컬 `HEAD`, `origin/main`, `git ls-remote --heads origin main`이 일치하고 `git status --short`가 빈 출력일 때만 완료로 판정합니다.

8. 사용자가 명시적으로 미추적 파일 생성을 허용하더라도 이는 작업 중 임시 생성을 허용한다는 뜻일 뿐, 작업 종료 후 방치를 허용하는 것으로 해석하지 않습니다.

9. 정상화 과정에서도 `git clean`, `git reset --hard`, 전체 `git add .`, 와일드카드 삭제, 파일 일괄 이동·삭제는 금지합니다. 각 파일을 확인한 뒤 정확한 경로 단위로 커밋 또는 `.gitignore` 처리합니다.

이 규칙을 지키지 않은 작업은 기능이 정상이어도 완료로 보고하지 않습니다. **최종 clean 상태가 아니면 "완료"라는 표현을 사용하지 않습니다.**
