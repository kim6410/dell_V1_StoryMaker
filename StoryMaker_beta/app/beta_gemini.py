...
- 기상 정보: 기상 서버 정보를 불러오지 못함 (날씨 항목 추정 금지)"""
    ...
    gemini_rule_block = """## Gemini 날짜·기상 우선 적용 규칙
- 서버가 제공한 날짜·요일·시간·기상정보를 현재 기준으로 사용한다.
- 입력 원문에 포함된 과거 날짜·요일·날씨 표현은 현장 기록으로만 취급한다.
- 현재 기상정보와 원문의 날씨 표현이 충돌하면 서버가 제공한 현재 기상정보를 우선한다.
- 날씨가 콘텐츠 주제와 자연스럽게 연결되지 않으면 억지로 언급하지 않는다.
- 기상정보가 제공되지 않은 항목은 추정하거나 만들어내지 않는다."""
