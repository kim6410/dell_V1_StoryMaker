...
### 키워드 밀도 및 지역 SEO 규칙
- 본문 전체에서 `[선택 지역명 + 주요 서비스]` 및 `[지역명 + 세부 작업]`(예: 울산 집수리, 울산 방화문 수리 등) 키워드 조합을 자연스러운 문맥으로 3~5회 적절히 반복 배치합니다.
- 입력 자료에 언급된 동 이름, 아파트 단지명(예: 야음동 하늘채센트럴파크, 에코하이츠아파트 등)을 본문 도입부와 현장 설명 부분에 명확히 명시합니다.

### 모바일 가독성 및 볼드체(Bold) 강조 규칙
- 독자가 훑어보아도(Scanning) 핵심을 파악할 수 있도록 아래 항목에 `**굵은 표시**`를 8회 이상 자연스럽게 적용합니다:
  1) 현장의 핵심 고장 증상 및 원인 (예: **가스쇼바 9개 압력 마모**, **힌지 및 수평 불균형**)
  2) 전문가의 핵심 조치 및 결과 (예: **안전 브라켓 고정 후 정교한 수평 조정**, **손으로 받치지 않아도 부드럽게 고정**)

## 전화번호 및 하단 문의(CTA) 규칙
- BLOG_POST, CARROT_POST, INSTAGRAM_POST, CAROUSEL_7, PODCAST_50, PODCAST_80에는 전화번호를 말미에 반드시 1회 넣습니다.
- BLOG_POST 최하단 문의 영역에는 단순히 전화번호만 적지 않고, "거주 지역(동/아파트명)과 현장 사진(전체 1장 + 가까이 1~2장)을 문자로 먼저 보내주시면 더욱 빠른 상담이 가능합니다." 문구와 함께 전화번호를 배치합니다.
...
- BLOG_TITLES 5개 제목 및 BLOG_POST 첫 줄 제목은 검색량이 많은 메인 검색어(`[지역명] + [동/아파트명] + [고장증상/작업내용]`)를 최우선으로 전진 배치합니다.
  (예시: "울산 야음동 현관방화문 처짐 수리, 헐거운 문도 부드럽게 해결해요")
...
- BLOG_HASHTAGS에는 메인 작업 태그 외에도, 동일 업종의 연관 서비스 태그(예: #울산중문수리 #울산도어락수리 #울산경첩교체 등)를 2~3개 추가하여 유입 범위를 확대합니다.
