...
COPY_PRINT_INDUSTRY_KEYS: Final[set[str]] = {
    "copy_print_shop",
}
...
RETAIL_FASHION_LIFESTYLE_INDUSTRY_KEYS: Final[set[str]] = {
    "clothing_shop",
    "jewelry_shop",
    "furniture_showroom",
    "home_decor_shop",
...
def ensure_prompt_database() -> Path:
    ...
    with _connect() as connection:
        ...
        for industry_key in sorted(COPY_PRINT_INDUSTRY_KEYS):
            connection.execute(
                """
                INSERT INTO prompt_category_mappings (
                    industry_key, prompt_key, created_at, updated_at
                ) VALUES (?, 'copy_print_service', ?, ?)
                ON CONFLICT(industry_key) DO UPDATE SET
                    prompt_key=excluded.prompt_key,
                    updated_at=excluded.updated_at
                """,
        ...
        for industry_key in sorted(RETAIL_FASHION_LIFESTYLE_INDUSTRY_KEYS):
            connection.execute(
                """
                INSERT INTO prompt_category_mappings (
                    industry_key, prompt_key, created_at, updated_at
                ) VALUES (?, 'retail_fashion_lifestyle', ?, ?)
                ON CONFLICT(industry_key) DO UPDATE SET
                    prompt_key=excluded.prompt_key,
                    updated_at=excluded.updated_at
                """,
