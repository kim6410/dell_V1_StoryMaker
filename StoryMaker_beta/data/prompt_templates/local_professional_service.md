...
- 현장 조건과 추가 점검에 따라 결과가 달라질 수 있으므로 `100% 해결`, `절대 재발 없음`, `완벽한 해결`, `반영구적 수명`, `무조건 보장`처럼 결과를 단정하거나 보증하는 표현을 사용하지 않습니다.
- 실제 입력자료에 보증 범위나 사후관리 조건이 명시된 경우에만 그 범위 안에서 정확히 안내합니다.
...
### 비포·애프터 및 작업 사진 활용 규칙
- 시공 전후 사진이나 작업 과정 사진이 실제 입력자료에 제공된 경우, 본문 중간에서 사진과 해당 작업 내용을 자연스럽게 연결합니다.
- 예시는 `위 사진에서 보시는 것처럼 시공 전에는 ~한 상태였지만, 작업 후에는 ~하게 정리되었습니다.`처럼 작성할 수 있습니다.
- 사진에 실제로 확인되는 범위만 설명하고, 보이지 않는 원인·상태·효과를 추정하지 않습니다.
- 사진이 제공되지 않았거나 사진 내용을 확인할 수 없으면 `위 사진`, `비포·애프터`, `사진에서 보시는 것처럼`이라는 표현을 사용하지 않습니다.
...
- CARROT_POST는 350자 이상 650자 이하로 작성합니다.
- 현장 사진이나 비포·애프터 이미지와 함께 읽기 좋도록 핵심 상황, 확인 내용, 작업 결과, 문의 안내를 짧은 문단으로 구성합니다.
