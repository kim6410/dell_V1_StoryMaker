(() => {
  ...
  function clearArchiveClientCache() {
    jobs = [];
    try {
      const keys = [];
      for (let i = 0; i < localStorage.length; i += 1) keys.push(localStorage.key(i));
      keys.filter((key) => key && /storymaker|archive|beta/i.test(key)).forEach((key) => localStorage.removeItem(key));
    } catch (_) {}
    try {
      const keys = [];
      for (let i = 0; i < sessionStorage.length; i += 1) keys.push(sessionStorage.key(i));
      keys.filter((key) => key && /storymaker|archive|beta/i.test(key)).forEach((key) => sessionStorage.removeItem(key));
    } catch (_) {}
    try { document.dispatchEvent(new CustomEvent('storymaker:archive-cache-cleared')); } catch (_) {}
  }
  ...
  async function req(url, options = {}) {
    ...
    if (response.status === 401 || response.status === 403) {
      clearArchiveClientCache();
      const error = new Error(data.detail || '로그인이 필요합니다.');
  ...
  async function load() {
    ...
    try {
      ...
      clearArchiveClientCache();
      list.className = 'empty';
      list.textContent = error.authRequired ? '로그인이 필요합니다. 로그인 후 보관함을 이용해 주세요.' : `보관함 조회 실패: ${error.message}`;
    }
  }
