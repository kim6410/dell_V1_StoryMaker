(() => {
  ...
  async function openDetail(jobId) {
    ...
    try {
      ...
      const order = (job.content?.channel_order || Object.keys(channels)).filter((key) => String(channels[key]?.content || '').trim());
      const first = order[0] || '';
      const mediaDeleted = Boolean(job.media_deleted_at);
      const deletedBytes = Math.max(0, Number(job.media_deleted_bytes || 0));
      const deletedSize = deletedBytes >= 1024 * 1024
        ? `${(deletedBytes / 1024 / 1024).toFixed(1)}MB`
        : deletedBytes >= 1024 ? `${(deletedBytes / 1024).toFixed(1)}KB` : `${deletedBytes}B`;
      const deleteReason = job.media_delete_reason === 'archive_limit' ? '보관 한도 자동 정리' : '사용자 파일 삭제';
      ...
      const channelSection = order.length ? `<section class="detail-block"><div class="channel-title-row"><h3>SNS 콘텐츠 기록</h3><span>저장된 채널 원고를 선택해 복사할 수 있습니다.</span></div><div class="channel-tabs">${order.map((key, index) => `<button type="button" class="channel-tab${index === 0 ? ' active' : ''}" data-channel="${esc(key)}">${esc(channels[key]?.label || key)}</button>`).join('')}</div><div id="archive-channel-content" class="channel-content rich-channel-content${first === 'BLOG' ? '' : ' archive-channel-dark'}">${first === 'BLOG' ? archiveCopyPayload(channels[first], first).rich : mobileCopyHtml(channels[first]?.content || '')}</div><div id="archive-smart-copy-bar" class="archive-smart-copy-bar"><div class="archive-smart-copy-info"><span class="archive-smart-copy-kicker">현재 채널</span><strong id="archive-smart-copy-label">${esc(channels[first]?.label || first)}</strong><small id="archive-smart-copy-meta">${archiveCopyPayload(channels[first], first).plain.length.toLocaleString('ko-KR')}자 · ${first === 'BLOG' ? '서식 유지 복사' : '텍스트 복사'}</small></div><button id="archive-channel-copy" type="button">${first === 'BLOG' ? '블로그 서식 복사' : `${esc(channels[first]?.label || first)} 복사`}</button></div></section>` : `<section class="detail-block deleted-empty-content"><h3>SNS 콘텐츠 기록</h3><p>저장된 채널 원고가 없습니다. 작업·사용량·과금 분석용 DB 기록은 유지됩니다.</p></section>`;
      const mediaSection = mediaDeleted ? `<section class="deleted-media-summary"><div class="deleted-media-heading"><span class="deleted-status-icon">✓</span><div><strong>저장 파일 정리 완료</strong><p>작업 목록과 DB 기록은 유지되고, 사용자 생성 파일만 삭제되었습니다.</p></div></div><dl><div><dt>삭제 일시</dt><dd>${esc(formatDate(job.media_deleted_at))}</dd></div><div><dt>확보 용량</dt><dd>${esc(deletedSize)}</dd></div><div><dt>삭제 사유</dt><dd>${esc(deleteReason)}</dd></div></dl><div class="deleted-asset-pills"><span>이미지 삭제됨</span><span>MP3 삭제됨</span><span>SRT 삭제됨</span><span>썸네일 삭제됨</span><span>MP4 삭제됨</span></div><p class="deleted-download-note">저장 파일이 삭제되어 미리보기와 ZIP 다운로드를 사용할 수 없습니다.</p></section>` : `<div class="archive-sections archive-sections-all"><div id="archive-media-all" class="archive-media-all"><section class="archive-section-static media-preview-card"><h3>업로드 이미지 ${images.length}장</h3><div class="image-grid">${images.map((_, index) => { const imageUrl = `/beta-api/browser/jobs/${encodeURIComponent(jobId)}/image/${index + 1}`; return `<button type="button" class="archive-media-thumb" data-media-preview="gallery" data-media-url="${imageUrl}" data-media-title="업로드 이미지 ${images.length}장" data-media-download="${esc(jobId)}_images.zip"><img loading="lazy" src="${imageUrl}" alt="이미지 ${index + 1}"></button>`; }).join('') || '<div class="empty-mini">이미지가 없습니다.</div>'}</div>${images.length ? `<div class="media-download-row"><a class="download-link" href="/beta-api/browser/jobs/${encodeURIComponent(jobId)}/images-download" download>다운로드</a></div>` : ''}</section><section class="archive-section-static media-preview-card"><h3>팟캐스트 MP3</h3>${audioUrl ? `<audio class="audio-preview" controls preload="metadata" src="${audioUrl}"></audio><div class="media-download-row"><a class="download-link" href="${audioUrl}" download>다운로드</a></div>` : '<div class="empty-mini">MP3가 없습니다.</div>'}</section><section class="archive-section-static media-preview-card"><h3>썸네일</h3>${thumbnailUrl ? `<button type="button" class="archive-media-thumb archive-media-thumb-large" data-media-preview="image" data-media-url="${thumbnailUrl}" data-media-title="썸네일 미리보기" data-media-download="${esc(jobId)}_thumbnail.png"><img class="thumbnail-preview" loading="lazy" src="${thumbnailUrl}" alt="썸네일"></button><div class="media-download-row"><a class="download-link" href="${thumbnailUrl}" download>다운로드</a></div>` : '<div class="empty-mini">썸네일이 없습니다.</div>'}</section><section class="archive-section-static media-preview-card"><h3>최종 MP4</h3>${videoUrl ? `<button type="button" class="archive-media-thumb archive-video-thumb" data-media-preview="video" data-media-url="${videoUrl}" data-media-title="최종 MP4 미리보기" data-media-download="${esc(jobId)}.mp4"><video class="mp4-preview" muted preload="metadata" playsinline src="${videoUrl}"></video><span>눌러서 크게 보기</span></button><div class="media-download-row"><a class="download-link" href="${videoUrl}" download>다운로드</a></div>` : '<div class="empty-mini">MP4가 없습니다.</div>'}</section></div><div class="archive-package-row"><button type="button" class="package-main-button" data-download-package="${esc(jobId)}">이미지 · MP3 · SRT · 썸네일 · MP4 전체 ZIP 다운로드</button></div></div>`;
      detail.innerHTML = `<div class="detail-head${mediaDeleted ? ' deleted-detail-head' : ''}"><div><div class="badge">${mediaDeleted ? 'BETA ARCHIVE RECORD' : 'BETA ARCHIVE DETAIL'}</div><h2 class="archive-detail-title">${esc(job.title || 'Beta 제작')}</h2><p>${esc(job.business?.name || '')} · ${esc(job.business?.region || '')} · ${esc(job.business?.service || '')}</p></div><button id="archive-detail-close" type="button">닫기</button></div>${mediaDeleted ? `<section class="deleted-record-banner"><strong>작업 기록 보존 중</strong><span>파일은 정리됐지만 사용량·추이·과금 분석을 위한 작업 DB는 그대로 유지됩니다.</span></section>` : ''}${channelSection}${mediaSection}`;
      ...
      detail.querySelectorAll('[data-media-preview]').forEach((button) => button.addEventListener('click', () => {
        ...
        openMediaPreview({type, url: button.dataset.mediaUrl, urls: galleryButtons.map((item) => item.dataset.mediaUrl), title: button.dataset.mediaTitle, downloadName: button.dataset.mediaDownload, downloadUrl: type === 'gallery' ? `/beta-api/browser/jobs/${encodeURIComponent(jobId)}/images-download` : ''});
      }));
    } catch (error) {
