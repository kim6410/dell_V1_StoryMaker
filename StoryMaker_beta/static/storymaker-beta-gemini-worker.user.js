...
(function () {
  ...
  function collectUploadPreviews() {
    const selectors = [
    ...
    return [...new Set(selectors.flatMap(queryAll))].filter(isVisible);
  }

  async function fetchImageAsFile(url, index) {
    const targetUrl = String(url || '').startsWith('http') ? String(url) : BACKEND + String(url || '');
    return new Promise((resolve, reject) => {
      GM_xmlhttpRequest({
        method: 'GET',
        url: targetUrl,
        ...
        onload: (res) => {
          if (res.status < 200 || res.status >= 300 || !res.response) {
            reject(new Error(`참고 이미지 다운로드 실패: HTTP ${res.status}`));
            return;
          }
          const blob = res.response;
          const type = blob.type || 'image/jpeg';
          const ext = type.includes('png') ? 'png' : type.includes('webp') ? 'webp' : 'jpg';
          resolve(new File([blob], `storymaker_beta_ref_${index + 1}.${ext}`, { type }));
        },
        onerror: () => reject(new Error('참고 이미지 다운로드 요청 실패'))
      });
    });
  }

  function dispatchUploadDrop(files) {
    const dataTransfer = new DataTransfer();
    files.forEach((file) => dataTransfer.items.add(file));
    const targets = [promptBox(), ...queryAll('rich-textarea'), ...queryAll('[contenteditable="true"]'), document.body].filter(Boolean);
  ...
  async function uploadImagesToGemini(imageUrls) {
    ...
    if (!urls.length) throw new Error('Beta 썸네일 참고 이미지 URL이 없습니다.');
    const files = await Promise.all(urls.map(fetchImageAsFile));
    const before = collectUploadPreviews().length;
    dispatchUploadDrop(files);
    for (let i = 0; i < 20; i++) {
      await sleep(500);
      if (collectUploadPreviews().length >= before + files.length) return true;
    }
    const fileInput = queryAll('input[type="file"]')
      .find((input) => String(input.accept || '').toLowerCase().includes('image') || !input.accept);
    if (fileInput) {
      const dt = new DataTransfer();
      files.forEach((file) => dt.items.add(file));
      try { fileInput.multiple = true; } catch (_) {}
    ...
    throw new Error('Gemini 참고 이미지 첨부 미리보기를 확인하지 못했습니다.');
  }
  ...
  async function runThumbnailJob(job) {
    ...
    try {
      let box = await waitPromptBox(55000);
      ...
      await uploadImagesToGemini(job.image_urls);
      await request('POST', '/beta-api/gemini-worker/thumbnail/ack', { job_id: jobId, status: 'uploaded', worker_id: workerId });
      box = await waitPromptBox(10000);
      if (!box) throw new Error('이미지 첨부 후 Gemini 입력창을 찾지 못했습니다.');
