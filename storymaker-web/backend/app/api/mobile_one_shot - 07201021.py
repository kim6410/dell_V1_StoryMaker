...
def _mobile_progress_payload(data: dict[str, Any], job_id: str, user_id: int) -> dict[str, Any]:
    ...
    status = str(data.get("status") or media.get("status") or pipeline.get("ai_worker_status") or "unknown")
    stage = str(data.get("stage") or media.get("message") or pipeline.get("stage") or "")
    percent = 0
    if status in {"completed", "shortform_completed", "gemini_completed"} or media.get("mp4_url"):
        percent = 100
    elif status in {"gemini_worker_waiting", "worker_queued"}:
        percent = 10
    elif status in {"gemini_completed", "podcast_script_ready"} or data.get("outputs"):
        percent = 45
    elif str(media.get("podcast_status") or "").lower() in {"submitted", "running"}:
        percent = 60
    elif str(media.get("shortform_status") or media.get("status") or "").lower().startswith("shortform"):
        percent = 75
    done_steps = sum(1 for step in steps if str(step.get("status") or "").lower() == "done") if isinstance(steps, list) else 0
    if steps:
        percent = max(percent, min(95, int(done_steps / max(1, len(steps)) * 100)))
    ...
    message = str(media.get("message") or pipeline.get("message") or stage or "작업 상태를 확인하고 있습니다.")[:1000]
    return {
        "ok": True,
        "job_id": job_id,
        "user_id": user_id,
        ...
        "message": message,
        "can_cancel": status in {"queued", "gemini_worker_waiting", "worker_queued"},
...
def _start_podcast_job(data: dict[str, Any], result_file: Path) -> dict[str, Any]:
    if not (data.get("raw_result") or data.get("outputs")):
        return data
    pipeline = data.get("pipeline") or {}
    script = str(pipeline.get("podcast_script") or "").strip()
    media = data.setdefault("media", {})
    # Browser MP3/SRT may finish before a delayed server-podcast submit path runs.
    # Never downgrade an already completed podcast back to submitted.
    if media.get("mp3_url") or media.get("mp3_path") or str(media.get("podcast_status") or "").lower() == "completed":
...
def _start_shortform_job(data: dict[str, Any], result_file: Path) -> dict[str, Any]:
    ...
    try:
        ...
        if not files:
            ...
            return data
        persona = data.get("persona") or {}
        current_job_id = str(data.get("job_id") or "mobile_one_shot")
        podcast_project_key = str(media.get("project_key") or current_job_id)
        ...
        # Keep the podcast asset fields untouched. Only the slideshow request
        # receives render-only copies with job-unique basenames. If the shared
        # renderer cannot see the V1 absolute path and falls back to a basename
        # lookup, it can no longer select another job's browser_podcast.mp3.
        render_mp3_path = str(media.get("mp3_path") or media.get("mp3_url") or "")
        render_srt_path = str(media.get("srt_path") or media.get("srt_url") or "")
        current_mp3_file = _first_mobile_media_file(media, "mp3", job_dir)
        if current_mp3_file:
            render_mp3_file = job_dir / "media" / f"render_audio_{_safe_filename(current_job_id, 'job')}.mp3"
            render_mp3_file.parent.mkdir(parents=True, exist_ok=True)
            if current_mp3_file.resolve() != render_mp3_file.resolve():
                shutil.copy2(current_mp3_file, render_mp3_file)
            render_mp3_path = str(render_mp3_file)
        current_srt_file = _first_mobile_media_file(media, "srt", job_dir)
        if current_srt_file:
            render_srt_file = job_dir / "media" / f"render_subtitle_{_safe_filename(current_job_id, 'job')}.srt"
            render_srt_file.parent.mkdir(parents=True, exist_ok=True)
            if current_srt_file.resolve() != render_srt_file.resolve():
                shutil.copy2(current_srt_file, render_srt_file)
            render_srt_path = str(render_srt_file)

        payload = {
            "project_key": current_job_id,
            "mp3_path": render_mp3_path,
            "srt_path": render_srt_path,
        ...
        response = httpx.post(f"{WORKER_API_URL}/api/slideshow/run", data=payload, files=files, headers=upstream_headers(), timeout=120)
        response.raise_for_status()
...
def _start_thumbnail_job(data: dict[str, Any], result_file: Path) -> dict[str, Any]:
    ...
    try:
        ...
        _make_mobile_thumbnail_collage(copied_image_paths, collage_path)
    ...
    reference_urls = [collage_url] if collage_url else copied_urls
    instagram_text = instagram_text[:3000]
    persona = data.get("persona") or {}
    project_title = str(persona.get("company_name") or data.get("job_id") or "모바일 원샷 썸네일")
    image_lines = "- 콜라주 참고 이미지: " + collage_url if collage_url else "\n".join(f"- {url}" for url in copied_urls)
    prompt = f"""[썸네일 제작 요청]
...
""".strip()
    ...
    media.update({
        ...
        "thumbnail_collage_url": collage_url,
        "thumbnail_collage_error": collage_error,
...
async def upload_mobile_one_shot_browser_podcast(
    ...
    mp3_path = media_dir / "browser_podcast.mp3"
    srt_path = media_dir / "browser_podcast.srt"
    mp3_path.write_bytes(mp3_content)
    if srt_content:
        srt_path.write_bytes(srt_content)

    safe_provider = provider if provider in {"webgpu", "wasm"} else "wasm"
    media = data.setdefault("media", {})
    v1_job_ref = quote(str(job_id), safe="")
    media.update({
        ...
        "srt_url": f"/v1-api/mobile/one-shot/jobs/{v1_job_ref}/files/srt" if srt_content else None,
        "status": "podcast_completed",
        "podcast_status": "completed",
...
def delete_mobile_one_shot_job_api(
    ...
    if result_file and result_file.exists():
        try:
            ...
            if not owner_matches and not (deleted_db and result_file_from_db):
                raise HTTPException(status_code=404, detail="삭제할 작업을 찾을 수 없습니다.")
            if owner_matches or result_file_from_db:
                data["deleted_at"] = _now().isoformat(timespec="seconds")
                data["status"] = "deleted"
                data["deleted_by"] = str(current_user.id)
                _write_json_atomic(result_file, data)
                deleted_file = True
    ...
    if not deleted_db and not deleted_file:
        raise HTTPException(status_code=404, detail="삭제할 작업을 찾을 수 없습니다.")
    return {"ok": True, "job_id": job_id, "deleted_db": deleted_db, "deleted_file": deleted_file}
