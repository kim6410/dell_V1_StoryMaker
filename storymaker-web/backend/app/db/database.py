...
def migrate_industry_prompt_templates() -> None:
    ...
    templates = [
        ...
        ("restaurant", "음식점", "외식·식음료", "음식, 온도, 향, 식감, 손님 분위기를 중심으로 작성합니다.", "계절감 → 메뉴 소개 → 식재료 → 조리 과정 → 맛 표현 → 재방문 유도", "맛집, 점심, 저녁, 가족외식, 메뉴명, 지역명", "따뜻하고 먹음직스러운 표현", "맛 보장, 건강 효능 단정"),
        ("meat_korean", "고깃집/한식", "외식·식음료", "불향, 식사 자리, 가족/단체 모임, 반찬 구성을 중심으로 작성합니다.", "방문 상황 → 대표 메뉴 → 굽는 장면 → 식사 분위기 → 재방문", "고깃집, 한식, 숯불, 회식, 가족외식", "정겹고 푸짐한 말투", "원산지나 품질 근거 없는 단정"),
        ("cafe", "카페", "외식·식음료", "공간 분위기, 커피 향, 디저트, 머무는 시간을 감성적으로 표현합니다.", "공간 분위기 → 향 → 음료 → 디저트 → 머무는 시간", "카페, 커피, 디저트, 분위기, 데이트", "차분하고 감성적인 말투", "지나친 감성 과장"),
        ("bakery_dessert", "베이커리/디저트", "외식·식음료", "갓 구운 향, 식감, 선물/간식 상황을 중심으로 작성합니다.", "제품 소개 → 식감 → 어울리는 상황 → 구매 유도", "베이커리, 디저트, 빵집, 케이크, 선물", "부드럽고 달콤한 표현", "원재료 효능 과장"),
        ("pub_bar", "이자카야/포차", "외식·식음료", "퇴근 후 분위기, 안주, 대화, 편안한 시간을 중심으로 작성합니다.", "하루 마무리 → 안주/메뉴 → 분위기 → 방문 유도", "이자카야, 포차, 술집, 안주, 모임", "편안하고 살짝 경쾌한 말투", "과음 조장"),
        ("mealkit_sidedish", "밀키트/반찬가게", "외식·식음료", "집밥, 간편함, 신선함, 가족 식탁을 중심으로 작성합니다.", "고민 상황 → 메뉴 구성 → 조리/보관 → 식탁 활용", "반찬가게, 밀키트, 집밥, 도시락, 간편식", "생활밀착형 친근한 말투", "건강 효능 단정"),
        ("workshop_class", "공방/원데이클래스", "라이프스타일·체험 및 공간", "체험 과정, 완성 결과, 추억과 선물 가치를 중심으로 작성합니다.", "참여 계기 → 체험 과정 → 완성품 → 추억", "공방, 원데이클래스, 체험, 취미, 만들기", "따뜻하고 섬세한 말투", "누구나 전문가처럼 된다는 과장"),
        ("partyroom_studio", "파티룸/스튜디오", "라이프스타일·체험 및 공간", "공간, 사진, 모임 목적, 예약 장점을 중심으로 작성합니다.", "모임 목적 → 공간 소개 → 이용 장면 → 예약 안내", "파티룸, 스튜디오, 생일파티, 촬영, 모임", "밝고 실용적인 말투", "수용 인원 과장"),
        ("camping", "캠핑장", "라이프스타일·체험 및 공간", "계절, 날씨, 자연 풍경, 가족·연인·친구 체험을 중심으로 작성합니다.", "도착 → 풍경 → 시설 → 체험 → 야경 → 추억 → 예약 유도", "캠핑장, 가족캠핑, 글램핑, 바베큐, 자연", "여행 에세이처럼 여유 있는 말투", "안전/시설 과장"),
