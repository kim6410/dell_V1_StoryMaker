...
function log(message, level = 'info') {
    ...
    logger.call(console, '[StoryMaker Auth]', message);
}

function notifyAuth(message) {
...
function ensureStoryMakerLoginModal() {
    if (window.StoryMakerV1AuthModal && typeof window.StoryMakerV1AuthModal.ensure === 'function') {
        return window.StoryMakerV1AuthModal.ensure();
    }
    throw new Error('V1 통합 인증 모달이 준비되지 않았습니다. 중복 모달은 생성하지 않습니다.');
...
function setStoryMakerAuthMode(tab) {
    if (window.StoryMakerV1AuthModal && typeof window.StoryMakerV1AuthModal.setMode === 'function') {
        return window.StoryMakerV1AuthModal.setMode(tab);
    }
...
function closeStoryMakerLoginModal() {
    if (window.StoryMakerV1AuthModal && typeof window.StoryMakerV1AuthModal.close === 'function') {
        return window.StoryMakerV1AuthModal.close();
    }
    const modal = document.getElementById('storymaker-login-modal');
    if (!modal) return;
    ...
    try {
        ...
        window.setTimeout(function() {
            try {
                ...
                window.location.replace(cleanUrl || '/v1/');
            } catch (e) {
...
function showAuthModal(tab) {
    if (window.StoryMakerV1AuthModal && typeof window.StoryMakerV1AuthModal.show === 'function') {
        return window.StoryMakerV1AuthModal.show(tab || 'login');
    }
...
function openAuthModalFromUrlAction() {
    try {
        // URL 기반 인증 모달 처리는 auth_modal_unified.js 한 곳에서만 담당합니다.
        // 두 스크립트가 동시에 action 파라미터를 처리하면 모달이 중복 또는 재노출될 수 있습니다.
        if (window.StoryMakerV1AuthModal) return;
...
async function completeAuthentication(authData, providerLabel) {
    ...
    notifyAuth(`${user.username}님, 반갑습니다!`);
...
async function handleGoogleCredential(googleResponse) {
    ...
    try {
        const response = await fetch('/v1-api/auth/google', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function handleLogout() {
    ...
    // 로그아웃 후 현재 메뉴를 유지하지 않고 V1 기본 대시보드로 돌아갑니다.
    window.location.replace('/v1/');
}
...
async function showMyPageModal() {
    ...
    log('마이페이지 정보 조회 중...');
    try {
        const response = await fetchWithAuth('/v1-api/auth/me');
        if (response.ok) {
            const res = await response.json();
            if (res.ok && res.data) {
                const user = res.data;
...
async function toggleWpEnabled(element) {
    ...
    try {
        const response = await fetchWithAuth('/v1-api/auth/settings', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
async function handleChangePassword(e) {
    ...
    log('비밀번호 변경 요청 중...');
    try {
        const response = await fetchWithAuth('/v1-api/auth/change-password', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        if (response.ok && res.ok) {
            ...
            notifyAuth('비밀번호 변경 완료. 다시 로그인해 주세요.');
            closeMyPageModal();
            handleLogout();
        } else {
...
async function checkLoginStatus() {
    ...
    if (!token) {
        ...
        try {
            const cookieResponse = await fetch('/v1-api/auth/me', {
                credentials: 'include'
            });
