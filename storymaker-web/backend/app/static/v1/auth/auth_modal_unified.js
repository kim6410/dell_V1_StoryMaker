...
(function(){
  ...
  function ensureCss(){
    ...
    link.rel = 'stylesheet';
    link.href = '/static/v1/auth/auth_modal_unified.css?v=20260727-v1-auth-final-1';
    document.head.appendChild(link);
  }

  function ensureStoryMakerLoginModal(){
    ensureCss();
    const authModals = Array.from(document.querySelectorAll('[id="storymaker-login-modal"]'));
    let modal = authModals.find(function(node){ return node.dataset && node.dataset.unifiedAuth === '1'; }) || null;
    authModals.forEach(function(node){ if (node !== modal) node.remove(); });
    ...
    modal.innerHTML = `
      <div class="sm-auth-card" role="dialog" aria-modal="true" aria-label="StoryMaker 통합 인증">
        ...
        <form id="storymaker-register-panel" class="sm-auth-panel">
          ...
          <div id="storymaker-register-message" class="sm-auth-message" role="status" aria-live="polite"></div>
          <button id="storymaker-register-submit" class="sm-auth-submit" type="submit">인증 메일 받기</button>
          <p class="sm-auth-note sm-auth-note-center">메일에서 비밀번호를 설정하면 가입이 완료됩니다.</p>
        </form>
  ...
  async function submitRegister(e){
    ...
    if (btn) { btn.disabled = true; btn.innerText = '인증 메일 발송 중...'; }
    try {
      ...
      showSuccess('인증 메일을 보냈습니다', data.message || '메일에서 비밀번호를 설정하면 가입이 완료됩니다.');
    } catch (err) {
      if (msg) {
      ...
      if (btn) { btn.disabled = false; btn.innerText = '인증 메일 받기'; }
    }
  }
  ...
  async function submitPasswordReset(e){
    ...
    const btn = document.getElementById('storymaker-password-submit');
    if (msg) { msg.classList.remove('is-error'); msg.style.display = 'none'; msg.innerText = ''; }
    if (!email) {
    ...
    if (btn) { btn.disabled = true; btn.innerText = '메일 발송 요청 중...'; }
    try {
      ...
      const data = await response.json().catch(function(){ return {}; });
      if (!response.ok || data.ok === false) throw new Error(data.detail || data.message || '비밀번호 재설정 요청에 실패했습니다.');
      showSuccess('메일을 확인해 주세요', data.message || '입력한 정보와 일치하는 계정이 있으면 비밀번호 재설정 메일을 발송했습니다.');
    } catch (err) {
      if (msg) {
        msg.innerText = err.message || '비밀번호 재설정 요청에 실패했습니다.';
        msg.classList.add('is-error');
        msg.style.display = 'block';
      }
    } finally {
      if (btn) { btn.disabled = false; btn.innerText = '재설정 링크 보내기'; }
    }
  }
  ...
  window.StoryMakerV1AuthModal = Object.freeze({
    ensure: ensureStoryMakerLoginModal,
    setMode: setStoryMakerAuthMode,
    show: showAuthModal,
    close: closeStoryMakerLoginModal,
    login: handleLogin,
    join: handleJoin
  });
  ...
  function consumeAuthActionParam(){
    try {
      const url = new URL(window.location.href);
      if (!url.searchParams.has('action')) return;
      url.searchParams.delete('action');
      const cleanUrl = `${url.pathname}${url.search}${url.hash}`;
      window.history.replaceState(window.history.state, '', cleanUrl || '/v1/');
    } catch(e) {}
  }
  ...
  function openFromAction(){
    try {
      ...
      if (action === 'login') { consumeAuthActionParam(); showAuthModal('login'); }
      else if (action === 'register' || action === 'join') { consumeAuthActionParam(); showAuthModal('register'); }
      else if (action === 'password' || action === 'lostpassword') { consumeAuthActionParam(); showAuthModal('password'); }
