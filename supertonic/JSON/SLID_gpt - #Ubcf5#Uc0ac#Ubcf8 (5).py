...
def build_filter_script_xfade(pre_imgs: list[Path], seg: float, fade: float, 
    ...
    for i in range(len(pre_imgs)):
        ...
        # 🔥 안정적 KenBurns zoompan 시스템 (완전 교체)
        frames = int(seg * settings.video.fps)
        frames = max(frames, 2)

        # ✅ (요청 반영) 줌 속도 1/5로 완화: 주기(period)를 5배로 늘립니다.
        period = frames * 5

        # ✅ 기본값(필요하면 UI 설정값으로 다음 패치에서 노출 가능)
        base_zoom = 1.20   # 모바일을 더 채우는 기본 확대(약 20%)
        zoom_amp  = 0.06   # 줌 진폭(생동감)
        pan_amp   = 0.03   # 줌팬 강도(조금 더 체감되게 상향)

        # ✅ zoompan 내부 변수: on(프레임 인덱스), zoom(현재 줌)
        # ✅ x/y는 (iw - iw/zoom) 범위를 기준으로 움직여야 "실제로" 팬이 됩니다.
        zoom = (
            f"zoompan="
            f"z='{base_zoom}+{zoom_amp}*sin(2*PI*on/{period})':"
            f"x='(iw-iw/zoom)/2 + (iw-iw/zoom)*{pan_amp}*sin(2*PI*on/{period})':"
            f"y='(ih-ih/zoom)/2 + (ih-ih/zoom)*{pan_amp}*cos(2*PI*on/{period})':"
        ...
        chain = (
            ...
            f"crop={settings.video.width}:{settings.video.height}:x='(in_w-out_w)/2':y='(in_h-out_h)*0.65',"
...
def build_video_onepass(pre_imgs: list[Path], audio_path: Path, srt_path: Path | None, 
    ...
    # ✅ 오디오 길이에 맞춰 이미지 반복 처리 (회전마다 랜덤 셔플)
    repeat_count = 1
    ...
    out_mp4 = output_folder / f"slideshow_{time.strftime('%Y%m%d_%H%M%S')}.mp4"
