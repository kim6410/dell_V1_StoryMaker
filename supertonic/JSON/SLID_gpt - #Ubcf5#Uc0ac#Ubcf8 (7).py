"""
SLID_gpt_STABLE_v5_1_ADVANCED.py
------------------------------------------------------------
목표(확정)
...
- ✅ 중앙 기준 10% 줌 (zoom_center_only 옵션)
- ✅ 줌팬 효과 (위험도 분석 및 선택 옵션)
"""
...
import queue
import json
SETTINGS_FILE = str(Path.home() / '.slid_gpt_settings.json')
...
def save_persisted_settings(cfg: dict):
    try:
...
class VideoSettings:
    ...
    zoom_intensity: float = 0.012
    zoom_center_only: bool = True  # 중앙 기준 10% 줌 (신규)
    enable_zoompam: bool = False    # 줌팬 효과 사용 (위험도 높음)
    zoompam_intensity: float = 0.005  # 줌팬 강도 (매우 약하게)
...
class WatermarkSettings:
    ...
    brand_plus_px: int = 8
    phone_plus_px: int = 5
    brand_color: str = "#FFD300"  # RGB Hex
...
class SubtitleSettings:
    ...
    font_size: int = 11
    biz_font_size: int = 34
    phone_font_size: int = 28
    bold: bool = True
    margin_v: int = 40
    outline: int = 4
    shadow: int = 2
...
class TransitionSettings:
    ...
    cycle_shuffle: bool = True  # 회전마다 랜덤 셔플
    reverse_cycle: bool = False  # 역순 재생
...
class EncodingSettings:
    ...
    x264_crf: int = 20
    x264_crf_optimized: int = 22  # 화질 손실 없이 용량 10% 감소 (CRF 20→22)
    audio_codec: str = "aac"
    audio_bitrate: str = "160k"  # 192k → 160k (용량 10% 감소)
    ...
    music_min_sec: float = 60.0  # 랜덤 음악이 짧으면 이 길이까지 여러 곡을 이어 붙임
...
def pick_random_audio(folder: str):
    """음악 폴더에서 랜덤 1곡 선택"""
    try:
        p = Path(folder)
...
def build_random_music_concat(ffmpeg_bin: str, ffprobe_bin: str, music_folder: str, target_sec: float,
                            temp_dir: Path, crossfade_sec: float = 0.25, qevt=None):
    """랜덤 음악을 여러 곡 이어붙여 target_sec 이상 만들고, 곡 사이 0.25초 크로스페이드로 자연스럽게 연결.
    최종 출력은 정확히 target_sec 길이의 mp3.
    """
    try:
        target_sec = float(target_sec or 0.0)
        if target_sec < 5.0:
            return None

        crossfade_sec = float(crossfade_sec or 0.25)
        crossfade_sec = max(0.0, min(crossfade_sec, 2.0))

        picked = []
        total = 0.0
        guard = 0

        # ✅ 크로스페이드는 곡 사이가 겹치므로 약간 더 길게 뽑아야 안전합니다.
        safety = crossfade_sec * 2.0

        while total < (target_sec + safety) and guard < 30:
            p = pick_random_audio(music_folder)
            if not p:
                break
            picked.append(p)
            dur = probe_audio_duration(Path(p), ffprobe_bin)
            if dur and dur > 0:
                total += float(dur)
            guard += 1
        ...
        out_mp3 = temp_dir / f"_rand_music_{int(target_sec)}s_xf{int(crossfade_sec*1000)}.mp3"

        if qevt:
            qevt.put(("log", f"[{_ts()}] 🎵 랜덤 음악 {len(picked)}곡 선택, 크로스페이드 {crossfade_sec:.2f}s"))
        ...
        # 2) filter_complex: acrossfade 체인 구성
        if len(picked) == 1 or crossfade_sec <= 0.0:
            # 단일 파일이면 그냥 자르기
            cmd += ["-t", f"{target_sec:.3f}", "-vn", "-c:a", "libmp3lame", "-q:a", "3", str(out_mp3)]
            run_ffmpeg_with_progress(cmd, qevt, label="audio")
            return str(out_mp3) if out_mp3.exists() else None

        # acrossfade는 각 입력이 같은 SR/ch이면 안정적이므로 aformat으로 통일
        filters = []
        for i in range(len(picked)):
            filters.append(f"[{i}:a]aformat=sample_rates=48000:channel_layouts=stereo,aresample=48000[a{i}]")
        last = "[a0]"
        for i in range(1, len(picked)):
            out = f"[ax{i}]"
            filters.append(f"{last}[a{i}]acrossfade=d={crossfade_sec}:c1=tri:c2=tri{out}")
            last = out
        ...
        if qevt:
            qevt.put(("log", f"[{_ts()}] ⚠ 랜덤 음악 크로스페이드 실패: {e}"))
        return None
...
def validate_settings(settings: AppSettings) -> List[str]:
    ...
    if settings.video.zoom_intensity > 0.025:
        warnings.append("⚠️ 줌 강도가 너무 높으면 멀미를 유발할 수 있습니다 (권장: 0.008~0.018)")
    elif settings.video.zoom_intensity < 0.005:
    ...
    if settings.video.enable_zoompam:
        ...
        if settings.video.zoompam_intensity > 0.01:
            warnings.append("⚠️ 줌팬 강도가 너무 높습니다. 0.005 이하로 낮추세요.")
...
def draw_watermark(canvas: Image.Image, settings: AppSettings) -> Image.Image:
    ...
    # 폰트 로드
    brand_font = load_font(int(wm.font_base + wm.brand_plus_px))
    phone_font = load_font(int(wm.font_base + wm.phone_plus_px))
...
def draw_text_bold(draw, xy, text, font, fill, bold_px=2):
    """상호를 굵게 보이게(겹쳐그리기)"""
    x, y = xy
    draw.text((x, y), text, font=font, fill=fill)
    for d in range(1, max(1, int(bold_px)) + 1):
        draw.text((x + d, y), text, font=font, fill=fill)
        draw.text((x, y + d), text, font=font, fill=fill)
        draw.text((x + d, y + d), text, font=font, fill=fill)
...
# =============================================================================
# FFmpeg filter script (이미지 회전마다 랜덤 셔플 + 중앙 줌 + 줌팬)
# =============================================================================
...
def build_filter_script_xfade(pre_imgs: list[Path], seg: float, fade: float, 
    ...
    for i in range(len(pre_imgs)):
        ...
        # 🔥 안정적 KenBurns (완전 교체 / 흔들림 최소화 / 워터마크 하단 보존)
        # - 1) 먼저 9:16 커버(cover)로 맞춘 뒤
        # - 2) zoompan에서 '하단 고정' 기준으로 아주 약하게 확대
        frames = int(seg * settings.video.fps)
        frames = max(frames, 2)

        # ✅ UI 줌 강도(예: 0.002) → 실제 줌 배율로 매핑(멀미 방지)
        zs = float(getattr(settings.video, "zoom_strength", 0.0) or 0.0)
        zs = clamp(zs, 0.0, 0.03)

        # ✅ 줌은 "매우 약하게" (요청: 속도 1/5 체감 = 변화량을 1/5로)
        #    예) zs=0.01이면 end_zoom ≈ 1.004 (아주 은은)
        start_zoom = 1.000
        end_zoom   = 1.000 + (zs * 0.20)

        # ✅ 워터마크 하단 보존: y를 (ih - ih/zoom)로 두면 '바닥'이 고정됩니다.
        # ✅ 흔들림 방지: x/y를 trunc()로 정수화합니다.
        pre_cover = (
            f"scale={settings.video.width}:{settings.video.height}:force_original_aspect_ratio=increase,"
            f"crop={settings.video.width}:{settings.video.height}:"
            f"x='trunc((in_w-out_w)/2)':"
            f"y='trunc(if(gt(in_h,out_h), in_h-out_h, 0))',"
            f"setsar=1,setdar={settings.video.width}/{settings.video.height},"
        )

        zoom = (
            f"zoompan="
            f"z='{start_zoom} + (min(on,{frames})/{frames})*({end_zoom-start_zoom})':"
            f"x='trunc((iw-iw/zoom)/2)':"
            f"y='trunc(ih-ih/zoom)':"
            f"d={frames}:s={settings.video.width}x{settings.video.height}:fps={settings.video.fps},"
        )
...
def build_video_onepass(pre_imgs: list[Path], audio_path: Path, srt_path: Path | None, 
    ...
    # ✅ 랜덤 음악 모드(크로스페이드): 선택된 오디오 길이(audio_len)에 맞춰 배경음악을 생성해 사용
    if getattr(settings.encoding, 'use_random_music', False) and getattr(settings.encoding, 'music_folder', ''):
        tmp_music = build_random_music_concat(settings.encoding.ffmpeg_bin, settings.encoding.ffprobe_bin,
                                             settings.encoding.music_folder, audio_len,
                                             temp_dir, crossfade_sec=0.25, qevt=qevt)
        if tmp_music:
            audio_path = Path(tmp_music)
            audio_len = probe_audio_duration(audio_path, settings.encoding.ffprobe_bin) or audio_len
            qevt.put(("log", f"[{_ts()}] 🎵 랜덤 음악 적용(크로스페이드): {audio_path.name}"))

    if audio_len <= 0:
        raise RuntimeError("오디오 길이 측정 실패. ffprobe 또는 오디오 파일 확인 필요.")
    
    # seg/fade 계산
    seg = float(settings.video.base_image_sec)
    fade = clamp(getattr(settings.transition, 'fade_sec', settings.transition.transition_ratio * seg), settings.video.transition_min_sec, settings.video.transition_max_sec)
    fade = min(fade, seg * 0.85)
    
    # 기본 영상 길이
    base_video_len = len(pre_imgs) * seg
    
    # ✅ 오디오 길이에 맞춰 이미지 반복 처리 (xfade 겹침까지 반영)
    # xfade는 (n*seg)에서 (n-1)*fade 만큼 겹치므로 실제 길이는:
    # total = n*(seg - fade) + fade
    repeat_count = 1
    if audio_len > 0:
        n0 = len(pre_imgs)
        if n0 >= 1:
            # ✅ fade가 seg보다 커지면 영상 길이가 오히려 줄어드는 '지뢰'가 됩니다.
            if fade >= seg * 0.90:
                fade = seg * 0.40
                qevt.put(("log", f"[{_ts()}] ⚠ 전환이 너무 길어(fade>=0.9*seg) fade를 {fade:.2f}s로 자동 조정"))
            step = max(seg - fade, 0.01)  # 각 추가 이미지가 늘려주는 순증가분
            # 필요한 총 이미지 수 N을 계산
            need_n = max(1, math.ceil((audio_len - fade) / step))
            # 반복 횟수로 변환
            repeat_count = max(1, math.ceil(need_n / n0))
            if repeat_count > 1:
                pre_imgs = build_cycle_shuffled_images(pre_imgs, repeat_count, settings.transition)
                qevt.put(("log", f"[{_ts()}] ✅ 오디오({audio_len:.2f}초) 기준 반복: {repeat_count}회 (xfade 반영)"))
    ...
    if settings.video.zoom_center_only:
        qevt.put(("log", f"[{_ts()}] 🎯 중앙 기준 10% 줌 적용"))
    elif settings.video.enable_zoompam:
        qevt.put(("log", f"[{_ts()}] ⚠️ 줌팬 효과 적용 (멀미 주의)"))
    ...
    out_mp4 = output_folder / f"{Path(base_dir).name}_{time.strftime('%Y%m%d_%H%M%S')}.mp4"
    ...
    def make_cmd(encoder: str) -> list[str]:
        ...
        if encoder == "h264_nvenc":
            cmd += ["-c:v", "h264_nvenc", "-preset", settings.encoding.nvenc_preset, 
                   "-pix_fmt", "yuv420p", "-cq", "22"]  # NVENC 화질 22 (용량 10% 감소)
        else:
            cmd += ["-c:v", "libx264", "-preset", settings.encoding.x264_preset, 
                   "-crf", str(settings.encoding.x264_crf_optimized),  # 22 사용
                   "-pix_fmt", "yuv420p"]
        
        # 오디오 (용량 최적화)
        cmd += ["-c:a", settings.encoding.audio_codec, "-b:a", settings.encoding.audio_bitrate]  # 160k
...
class SettingsFrame(ttk.Frame):
    ...
    def create_video_tab(self, notebook):
        ...
        # 줌 강도
        ttk.Label(frame, text="줌 강도 (0.005~0.03):").grid(row=row, column=0, sticky="w", padx=5, pady=2)
        var = tk.DoubleVar(value=self.settings.video.zoom_intensity)
        var.trace_add("write", self.on_setting_changed)
        self.vars["video.zoom_intensity"] = var
        ttk.Spinbox(frame, from_=0.0, to=0.05, increment=0.001, textvariable=var, width=10).grid(row=row, column=1, padx=5, pady=2)
        row += 1
        ...
        ttk.Spinbox(frame, from_=0.001, to=0.01, increment=0.001, textvariable=var, width=10).grid(row=row, column=1, padx=5, pady=2)
        row += 1
    ...
    def create_watermark_tab(self, notebook):
        ...
        # 폰트 크기 기본
        ttk.Label(scrollable_frame, text="폰트 기본 크기:").grid(row=row, column=0, sticky="w", padx=5, pady=2)
        var = tk.IntVar(value=self.settings.watermark.font_base)
        var.trace_add("write", self.on_setting_changed)
        self.vars["watermark.font_base"] = var
        ttk.Spinbox(scrollable_frame, from_=20, to=80, increment=2, textvariable=var, width=10).grid(row=row, column=1, padx=5, pady=2)
        row += 1
        ...
        ttk.Checkbutton(scrollable_frame, variable=var).grid(row=row, column=1, sticky="w", padx=5, pady=2)
        row += 1
...
# =============================================================================
# 멀티 진행바 컴포넌트 (상세 정보 포함)
# =============================================================================

class MultiProgressBar(tk.Frame):
    """전처리/인코딩 분리 진행바 + 상세 정보"""
...
class CineUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("시네마틱 숏폼 제작기 v5.1")
    ...
    def create_widgets(self):
        ...
        tk.Label(header, text="시네마틱 숏폼 제작기 v5.1", fg="#ffffff", bg="#0f1115",
                 font=("Malgun Gothic", 18, "bold")).pack(anchor="w")
        tk.Label(header, text="1080x1920 / 반사배경 / 워터마크 / 자막 / 회전마다 랜덤 / 용량 최적화 / 중앙줌",
    ...
    def select_audio_srt_pair(self):
        """오디오와 자막 파일 동시 선택 (파일명 동일, 확장자만 다른 경우)"""
        # 오디오 파일 먼저 선택
