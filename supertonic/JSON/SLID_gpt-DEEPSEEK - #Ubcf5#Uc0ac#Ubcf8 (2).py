"""
SLID_gpt_STABLE_v5_PHASE1_COMPLETE.py
------------------------------------------------------------
목표(확정)
...
def preprocess_images(src_folder: Path, settings: AppSettings, qevt: queue.Queue, preview_only: bool = False):
    ...
    for i, p in enumerate(imgs, start=1):
        ...
        qevt.put(("progress_pre", f"{i}/{total} 처리 중", progress * 100, eta))
...
# =============================================================================
# FFmpeg filter script (이미지 회전마다 랜덤 셔플)
# =============================================================================
...
def build_filter_script_xfade(pre_imgs: list[Path], seg: float, fade: float, 
    ...
    for i in range(len(pre_imgs)):
        ...
        zoom = (
            f"scale=iw*(1+{settings.video.zoom_intensity:.6f}*t):"
            f"ih*(1+{settings.video.zoom_intensity:.6f}*t):eval=frame,"
        )
...
def build_video_onepass(pre_imgs: list[Path], audio_path: Path, srt_path: Path | None, 
    ...
    qevt.put(("progress_enc", "인코딩 준비 중...", 0, audio_len))
    ...
    for label, enc in (("NVENC", settings.encoding.enc_primary), 
        ...
        if rc == 0:
            ...
            qevt.put(("progress_enc", "인코딩 완료", 100, 0))
...
class MultiProgressBar(tk.Frame):
    ...
    def __init__(self, parent):
        ...
        self.eta_label = tk.Label(info_frame, text="⏱️ 예상시간: --:--", 
                                   fg="#c7d0db", bg="#0f1115", font=("Malgun Gothic", 9))
        self.eta_label.pack(side="left")
        
        self.speed_label = tk.Label(info_frame, text="⚡ 속도: --", 
                                     fg="#c7d0db", bg="#0f1115", font=("Malgun Gothic", 9))
        self.speed_label.pack(side="right")
    
    def update_preprocess(self, current: int, total: int, percent: float, eta: float):
        """전처리 진행률 업데이트"""
    ...
    def update_encode(self, percent: float, eta: float, current_sec: float = None, total_sec: float = None):
        """인코딩 진행률 업데이트"""
        ...
        if eta > 0:
            self.eta_label.config(text=f"⏱️ 남은시간: {format_time(eta)}")
    ...
    def reset(self):
        ...
        self.eta_label.config(text="⏱️ 예상시간: --:--")
        self.speed_label.config(text="⚡ 속도: --")
...
class CineUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("시네마틱 숏폼 제작기 v5")
    ...
    def create_widgets(self):
        ...
        tk.Label(header, text="시네마틱 숏폼 제작기 v5", fg="#ffffff", bg="#0f1115",
                 font=("Malgun Gothic", 18, "bold")).pack(anchor="w")
        tk.Label(header, text="1080x1920 / 반사배경 / 워터마크 / 자막 / 회전마다 랜덤 / 용량 최적화",
        ...
        self.txt = ScrolledText(log_frame, wrap="word", height=10, bg="#121622", fg="#dbe5f0",
        ...
        try:
            while True:
                ...
                if kind == "log":
                    ...
                    _, detail, percent, eta = evt
                    # detail에서 현재/전체 파싱
                    match = re.search(r"(\d+)/(\d+)", detail)
                    if match:
                        current = int(match.group(1))
                        total = int(match.group(2))
                        self.progress_bars.update_preprocess(current, total, percent, eta)
                    ...
                    if match:
                        ...
                        self.progress_bars.update_encode(percent, eta, current, total)
                    else:
                        self.progress_bars.update_encode(percent, eta)
                    self.set_progress("인코딩", detail)
                elif kind == "done":
                    _, report = evt
                    self.log("✅ 작업 완료.")
                    self.progress_bars.update_encode(100, 0)
