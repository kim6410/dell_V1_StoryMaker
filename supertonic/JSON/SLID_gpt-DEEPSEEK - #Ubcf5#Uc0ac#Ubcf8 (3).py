...
def build_filter_script_xfade(pre_imgs: list[Path], seg: float, fade: float, 
    ...
    for i in range(len(pre_imgs)):
        ...
        if settings.video.zoom_center_only:
            # 중앙 기준 10% 줌 (zoompan 없이 scale만)
            zoom = (
                f"scale=iw*(1+{zoom_factor:.6f}*t):"
                f"ih*(1+{zoom_factor:.6f}*t):eval=frame,"
            )
        elif settings.video.enable_zoompam:
            # 줌팬 효과 (중앙에서 시작해서 랜덤 방향으로 이동)
            # 랜덤 방향 생성 (각 이미지마다 다르게)
            pan_x = random.uniform(-0.3, 0.3)  # 좌우 이동 범위
            pan_y = random.uniform(-0.3, 0.3)  # 상하 이동 범위
            
            zoom = (
                f"zoompan=z='min(zoom+{settings.video.zoompam_intensity:.6f},1.1)':"
                f"x='iw/2-(iw/zoom/2)+{pan_x:.3f}*t':"
                f"y='ih/2-(ih/zoom/2)+{pan_y:.3f}*t':"
                f"d={int(seg*settings.video.fps)}:s={settings.video.width}x{settings.video.height},"
            )
        else:
            # 기본 줌 (중앙)
            zoom = (
        ...
        chain = (
            f"[{i}:v]"
            f"{zoom}"
            f"scale={settings.video.width}:{settings.video.height}:force_original_aspect_ratio=decrease,"
