...
class EncodingSettings:
    ...
    x264_crf: int = 20
    x264_crf_optimized: int = 28  # 🔥 22 → 28 (모바일 숏폼용 15-20MB 목표)
    audio_codec: str = "aac"
    audio_bitrate: str = "128k"   # 🔥 160k → 128k (약간 다이어트)
...
def build_video_onepass(pre_imgs: list[Path], audio_path: Path, srt_path: Path | None, 
    ...
    def make_cmd(encoder: str) -> list[str]:
        ...
        # 🔥 모바일 숏폼용 최적화 설정 (CRF 28 + 오디오 128k)
        if encoder == "h264_nvenc":
            cmd += ["-c:v", "h264_nvenc", "-preset", settings.encoding.nvenc_preset, 
                   "-pix_fmt", "yuv420p", "-cq", "28"]  # CQ 28 (모바일 최적화)
        else:
            cmd += ["-c:v", "libx264", "-preset", settings.encoding.x264_preset, 
                   "-crf", str(settings.encoding.x264_crf_optimized),  # CRF 28
                   "-pix_fmt", "yuv420p"]
        
        cmd += ["-c:a", settings.encoding.audio_codec, "-b:a", settings.encoding.audio_bitrate]  # 128k
...
class SettingsFrame(ttk.Frame):
    ...
    def create_advanced_tab(self, notebook):
        ...
        ttk.Label(frame, text="CRF 28 / 오디오 128k (모바일 숏폼용)", 
