...
class VideoSettings:
    ...
    zoom_intensity: float = 0.003  # 매우 미세한 줌
...
def build_video_onepass(pre_imgs: list[Path], audio_path: Path, srt_path: Path | None, 
    ...
    base_video_len = original_img_count * seg
    ...
    if audio_len > 0 and base_video_len < audio_len:
        ...
        qevt.put(("log", f"  - 계산된 영상 길이: {len(pre_imgs) * seg:.2f}초"))
    ...
    for label, enc in (("NVENC", settings.encoding.enc_primary), 
        ...
        qevt.put(("log", f"[{_ts()}] 실행: {label} (모바일 최적화 CRF 28)"))
        ...
        if rc == 0:
            ...
            qevt.put(("log", f"[{_ts()}] ✅ 파일 크기: {file_size_mb:.2f} MB (CRF 28 적용)"))
...
class CineUI:
    ...
    def create_widgets(self):
        ...
        tk.Label(header, text="시네마틱 숏폼 제작기 v5.2", fg="#ffffff", bg="#0f1115", font=("Malgun Gothic", 18, "bold")).pack(anchor="w")
        ...
        tk.Label(preview_frame, text="🖼️ 프리뷰 (첫 번째 이미지)", fg="#ffffff", bg="#1a1e2a", font=("Malgun Gothic", 12, "bold")).pack(anchor="w", padx=10, pady=(10, 5))
        
        self.preview_canvas = tk.Canvas(preview_frame, bg="#121622", width=360, height=640, highlightthickness=0)
        self.preview_canvas.pack(pady=10, padx=10)
        
        self.preview_info = tk.Label(preview_frame, text="이미지 폴더를 선택하세요", fg="#9aa4b2", bg="#1a1e2a", font=("Malgun Gothic", 9))
    ...
    def _report_popup(self, report: BuildReport):
        ...
        tk.Label(pop, text="✅ 작업 완료", fg="#ffffff", bg="#0f1115", font=("Malgun Gothic", 14, "bold")).pack(anchor="w", padx=14, pady=(12, 0))
        ...
        msg = (
            ...
            f"📦 파일 크기: {report.file_size_mb:.2f} MB (CRF 28 적용)\n"
